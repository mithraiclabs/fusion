...
const IndexIntroUser = () => {
  ...
  const [mintInfos, setMintInfos] = useState<Record<string,MintInfo>>({});
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      const mints = await loadMintInfo(provider.connection, Object.values(projectOptions));
      setMintInfos(mints);
