...
import { optionMarketFamily, psyAmericanOptionKeys, TokenAccount } from "../../recoil";
...
export const selectOwnedProjectOptionKeys = selector<OwnedProjectOptionKeys>({
  key: "selectOwnedProjecctOptions",
  get: ({get}) =>{
    ...
    return psyAmericanOptions.reduce((agg, optionMarket) => {
      ...
      const tokenAccount = get(tokenAccountsMap(optionMarket.optionMint.toString()))
